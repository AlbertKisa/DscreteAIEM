...
int main()
{
	...
	for (i = 0; i <= nodetime; i++)
	{
		...
		outfile_time << tt * 1000 << "        " << a1vv << "        " << a2vv << "        " << a3vv << "        " << a1hh << "        " << a2hh << "        " << a3hh << endl;
	}
	for (f = -200; f <= 200; f++)
	{
		...
		for (i = 1; i <= nodetime; i++)
		{
			...
			tempvv = tempvv + delt_t*timevv[i] * exp(complex<double>(0.0, -2 * PI*f*delt_t*i));
			temphh = temphh + delt_t*timehh[i] * exp(complex<double>(0.0, -2 * PI*f*delt_t*i));
