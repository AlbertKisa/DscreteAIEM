...
void IFT_mathematica(int Num, complex<double> *Matrix_ori,
                     complex<double> *ISAR1_t1) {
  for (int k = 0; k < Num; k++) {
...
void IFT_plus(int Num, complex<double> *Matrix_int,
              complex<double> *Matrix_out) {
  int Num1 = 2 * Num + 1;
  ...
  for (int i = 0; i < Num; i++) {
    Matrix_out[i] =
        1.0 / sqrt(Num1) *
        exp(complex<double>(0, 2 * PI * Num / Num1 * (i + Num + 1))) *
        Matrix_temp[i + Num + 1];
  }
  for (int j = Num; j < Num1; j++) {
    Matrix_out[j] =
        1.0 / sqrt(Num1) *
        exp(complex<double>(0, 2 * PI * Num / Num1 * (j + Num + 1))) *
        Matrix_temp[j - Num];
  }
...
void Ints(int Num, double delt, complex<double> **Matrix_int,
          complex<double> &Sum) {
  Sum = 0;
...
void Intk(int Num, double delt, complex<double> **Matrix_int,
          complex<double> &Sum) {
  Sum = 0;
  for (int i = 0; i < 2 * Num + 1; i++) {
    for (int j = 0; j < 2 * Num + 1; j++) {
...
void SSA1_linear(complex<double> eps_r, double vk, double the_s, double phi_s,
  ...
  for (ii = 0; ii < Num1; ii++) {
    for (jj = 0; jj < Num1; jj++) {
      ...
      G_xyz = exp(-(((ii - Num) * delt) * ((ii - Num) * delt) +
                    ((jj - Num) * delt) * ((jj - Num) * delt)) /
                  gt / gt / pow(cos(the_i), 2));

      Ker3[ii][jj] = (exp(pow(q0 + q01, 2) * real(Ker2[ii][jj])) - 1) *
                     exp(-pow(q0 + q01, 2) * Sum1) *
                     exp(complex<double>(0.0, -kr * delt)) * G_xyz;
    }
  }
  ...
  b[0] = 4 * PI * PI / (pow(2 * PI, 4)) * 4 * q1 * q01 / (pow(q1 + q01, 2)) *
         pow(abs(B1vv), 2) * Sum2;
  b[1] = 4 * PI * PI / (pow(2 * PI, 4)) * 4 * q1 * q01 / (pow(q1 + q01, 2)) *
         pow(abs(B1hh), 2) * Sum2;
}
