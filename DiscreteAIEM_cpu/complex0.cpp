...
//double x,y;
{
	complex0 z; z.x = x; z.y = y; return z;
}
...
// complex z;
{
	complex0 c;
	float xx, yy, w, r;
	if ((z.x == 0.0) && (z.y == 0.0)) { c.x = 0.0; c.y = 0.0; return c; }
	else {
		xx = fabs(z.x); yy = fabs(z.y);
		if (xx >= yy) { r = yy / xx; w = sqrt(xx)*sqrt(0.5*(1.0 + sqrt(1.0 + r*r))); }
		else  { r = xx / yy;   w = sqrt(yy)*sqrt(0.5*(r + sqrt(1.0 + r*r))); }
		if (z.x >= 0.0)  { c.x = w; c.y = z.y / (2.0*w); }
		else  { c.y = (z.y >= 0) ? w : -w;     c.x = z.y / (2.0*c.y); }
		return c;
	}
}
//*******************************************************************
...
{
	complex0 z; z.x = 0.0; z.y = 0.0; return z;
}
...
{  //V=CMPLX_Matrix(nRow,nCol);
	for (ii1 = 0; ii1<nRow; ii1++)
	for (jj1 = 0; jj1<nCol; jj1++)
		V[ii1][jj1] = COMplex_Null();
